...
def _filter_feasible(pop):
    return [p for p in pop if is_feasible_cost(p.get('Cost', None))]


def _build_model_for_plot(model):
    return {
        'H': model['H'],
...
def _empty_result(mode, model):
    model_for_plot = _build_model_for_plot(model)
    if mode == 'stats':
        return np.empty((0, 4))
    if mode == 'visual':
        return None, model_for_plot
    if mode == 'full':
        return [], [], model_for_plot
...
def run_aimopso(
    ...
    """Run the A*IMOPSO optimizer.

    Returns:
    - `stats`: `(n, 4)` cost array
    - `visual`: `(best_path_xyz, model_for_plot)`
    - `full`: `(all_paths_xyz, feasible_repository, model_for_plot)`
    """
    if seed is not None:
        np.random.seed(seed)
        random.seed(seed)
    ...
    template_pos = None
    if use_a_star_init:
        print('    -> Trying to build an A* guidance path for initialization...')
        guidance_path_xy = get_a_star_guidance_path(model)
        if guidance_path_xy is not None and len(guidance_path_xy) > 0:
            guidance_path_xy = np.asarray(guidance_path_xy, dtype=float)
            safe_avg_altitude = (model['zmax'] + model['zmin']) / 2.0
            guidance_path_z_rel = np.full(len(guidance_path_xy), safe_avg_altitude)
            guidance_path_xyz_rel = np.hstack(
                [guidance_path_xy, guidance_path_z_rel.reshape(-1, 1)]
            )
            template_pos = cartesian_to_spherical(guidance_path_xyz_rel, model)
        else:
            print('    -> A* guidance path is empty. Falling back to random initialization.')
    else:
        print('    -> A* guidance is disabled. Falling back to random initialization.')
    ...
    for i in range(n_pop):
        ...
        for key in ['r', 'psi', 'phi']:
            pos[key] = np.clip(pos[key], var_min[key], var_max[key])

        vel = {key: np.zeros(var_size) for key in ['r', 'psi', 'phi']}
        cost = calculate_cost(
            spherical_to_cartesian(pos, model),
            model,
        ...
        particles.append(
            {
                ...
                'Cost': cost,
                'Best': {
                    'Position': {k: v.copy() for k, v in pos.items()},
                    'Cost': None if cost is None else cost.copy(),
                },
                'CrowdingDistance': 0,
            }
        )
    ...
    rep = get_new_repository(init_pool, n_rep) if init_pool else []

    if template_pos is not None:
        print(f'    [A*IMOPSO] A* guided initialization complete: {n_pop} particles, feasible: {len(init_pool)}')
        print(f'               |- 5% pure A* paths: {n_pure_astar}')
        print(f'               |- 50% low-noise variants: {n_low_noise}')
        print(f'               `- 45% normal-noise variants: {n_normal_noise}')
    ...
    for _ in range(max_it):
        ...
        for p in particles:
            ...
            if use_dual_leader:
                if np.random.rand() < prob_attack_force:
                    leader = global_best_leader
                else:
            ...
            for key in ['r', 'psi', 'phi']:
                r1 = np.random.rand(*var_size)
                r2 = np.random.rand(*var_size)
                p['Velocity'][key] = (
                    w * p['Velocity'][key]
                    + c1 * r1 * (p['Best']['Position'][key] - p['Position'][key])
                    + c2 * r2 * (leader['Position'][key] - p['Position'][key])
                )
            ...
            p['Cost'] = calculate_cost(
                spherical_to_cartesian(p['Position'], model),
                model,
                terrain_interpolator,
                var_min,
            )
            if (
                p['Best']['Cost'] is None
                or dominates(p['Cost'], p['Best']['Cost'])
                or (
                    not dominates(p['Best']['Cost'], p['Cost'])
        ...
        for p in particles:
            ...
            mutated_particles.append(
                {
                    'Position': new_pos,
                    'Velocity': {k: v.copy() for k, v in p['Velocity'].items()},
                    'Cost': calculate_cost(
                        spherical_to_cartesian(new_pos, model),
                        model,
                        terrain_interpolator,
                        var_min,
                    ),
                    'Best': {
                        'Position': {k: v.copy() for k, v in new_pos.items()},
                        'Cost': np.inf,
                    },
                    'CrowdingDistance': 0,
                }
        ...
        rep = get_new_repository(candidate_pool, n_rep) if candidate_pool else []
        w *= wdamp

    if not rep:
        return _empty_result(mode, model)
    ...
    if mode == 'visual':
        best_solution = rep_feasible[
            np.argmin(np.sum(np.array([p['Cost'] for p in rep_feasible]), axis=1))
        ]
        final_cart = spherical_to_cartesian(best_solution['Position'], model)
        path_points_waypoints_only = np.column_stack(
            [final_cart['x'], final_cart['y'], final_cart['z']]
        )
