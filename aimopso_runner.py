...
def _reviewer_cli():
    import argparse
    import os
    from environments import create_scene_model

    parser = argparse.ArgumentParser(description="AIMOPSO minimal reviewer demo runner")
    parser.add_argument("--scene", type=int, default=1, choices=[1, 2, 3, 4], help="Scene ID")
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument("--max-it", dest="max_it", type=int, default=30, help="Max iterations")
    parser.add_argument("--n-pop", dest="n_pop", type=int, default=80, help="Population size")
    parser.add_argument("--n-rep", dest="n_rep", type=int, default=40, help="Repository size")
    args = parser.parse_args()

    model = create_scene_model(args.scene)
    costs = run_aimopso(
        model=model,
        seed=args.seed,
        mode='stats',
        max_it_override=args.max_it,
        n_pop_override=args.n_pop,
        n_rep_override=args.n_rep,
    )

    os.makedirs("results/demo", exist_ok=True)
    out_path = f"results/demo/latest_run_scene{args.scene}.csv"

    if costs is None or len(costs) == 0:
        with open(out_path, "w", encoding="utf-8") as f:
            f.write("J1,J2,J3,J4\n")
        print(f"No feasible solutions found. Empty output saved to: {out_path}")
        return

    np.savetxt(out_path, costs, delimiter=",", header="J1,J2,J3,J4", comments="")
    print(f"Saved {len(costs)} Pareto points to: {out_path}")
